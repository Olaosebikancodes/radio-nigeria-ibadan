    ...
    <AdminLayout>
      <div style={{ padding:'32px' }}>
        ...
        <div style={{ background:'var(--color-surface)', borderRadius:'16px', border:'1px solid var(--color-border)', overflow:'hidden' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 120px 100px 120px 100px', gap:'16px', padding:'12px 20px', background:'var(--color-surface-2)', borderBottom:'1px solid var(--color-border)' }}>
          ...
          {loading ? <p style={{padding:'24px',color:'var(--color-text-muted)'}}>Loading…</p>
            ...
            : articles.map(a=>(
              <div key={a.id} style={{ display:'grid', gridTemplateColumns:'1fr 120px 100px 120px 100px', gap:'16px', padding:'14px 20px', borderBottom:'1px solid var(--color-border)', alignItems:'center' }}
              ...
              >
                ...
                <p style={{ fontSize:'12px', color:'var(--color-text-muted)', textTransform:'capitalize' }}>{a.category||'—'}</p>
                <p style={{ fontSize:'12px', color:'var(--color-text-muted)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{a.stations?.name||'—'}</p>
                <p style={{ fontSize:'12px', color:'var(--color-text-dim)' }}>{formatDate(a.published_at||a.created_at)}</p>
