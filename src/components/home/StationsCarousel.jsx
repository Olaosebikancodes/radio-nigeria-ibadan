...
function StationChip({ station }) {
  ...
  const isActive =
    activeStation?.id === station.id || activeStation?.slug === station.slug;
  ...
  return (
    ...
    >
      ...
      >
        {station.name.replace(" FM", "")}
        <span style={{ color: color }}> FM</span>
      </p>
      <p
        style={{
          fontSize: "14px",
...
export default function StationsCarousel() {
  ...
  useEffect(() => {
    supabase
      ...
      .order("sort_order")
      .then(({ data }) => {
        setStations(data?.length ? data : STATIONS_SEED);
        setLoading(false);
      });
  }, []);
          ...
          ? Array(8)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className="skeleton"
                  style={{ height: "178px", borderRadius: "16px" }}
                />
              ))
          : stations.map((s) => (
              <StationChip key={s.id ?? s.slug} station={s} />
            ))}
      </div>
